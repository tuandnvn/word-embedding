...
from word2vec_retrain import Word2Vec_Retrain
...
#         using_pos = False
    ...
    original_word2vec_model_file = outf('w2v_%s_pos_f_%s' % (FILE_SIZE, TOKEN_LIMIT))
    word2vec_model_file = outf('w2v_retrain_bnc_%s_pos_f_%s' % (BNC_FILE_SIZE, TOKEN_LIMIT))
    prototype_file = outf('prototype.pos.json')
    output_prototype_file = outf('prototype.out.pos.json')
    using_pos = True
    ...
    if os.path.exists(word2vec_model_file):
        ...
        model = Word2Vec_Retrain( initial_model, prototype, size=300, min_count=0, window=5, workers=8, negative=5, iter=1)
