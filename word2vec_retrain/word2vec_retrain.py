...
#         self.save_prototype_text('1.txt')
        ...
        self.no_of_samples = defaultdict(int)
        self.no_of_learnt_samples = defaultdict(int)
        self.margins = defaultdict(list)
        self.margin_alpha = defaultdict(float)
        
        for target_word in self.prototypes:
            self.margin_alpha[target_word] = 1.0
            
        self.cutout_threshold = 0.95
...
#             print 'target_word %s has %d patterns' % ( target_word, len(self.prototypes[target_word]) ) 
    ...
    def reset_weights(self):
        ...
        self.syn0prime_normalized = empty( (self.no_prototypes, self.layer1_size), dtype=REAL)
        for target_word in self.prototypes:
            target_word_w_pos = target_word + '-v'
            if target_word_w_pos in self.vocab:
                for pattern_no in self.prototypes[target_word]:
                    ...
                    self.syn0prime_normalized[index] = self.prototypes[target_word][pattern_no] / numpy.linalg.norm(self.syn0prime[index])
    ...
    def update_prototype_to_current(self):
        print 'Update total numbers'
        for target_word in self.no_of_learnt_samples:
            print 'Word %s, percentage = %f' % (target_word , 100.0 * self.no_of_learnt_samples[target_word] / self.no_of_samples[target_word])
            
        print 'Margin alpha'
        for target_word in self.no_of_learnt_samples:
            print 'Word %s, margin alpha = %f' % (target_word , self.margin_alpha[target_word])
    ...
    def select_train_sentence_sg(self, sentence, alpha, work):
        ...
        '''Re-normalize self.syn0prime'''
        for index in xrange(self.no_prototypes):
            self.syn0prime_normalized[index] = self.syn0prime[index] / numpy.linalg.norm(self.syn0prime[index])
            
            
        '''Update margin alpha'''
        for target_word in self.prototypes:
            self.margins[target_word] = sorted(self.margins[target_word])
            cut_out = int (len(self.margins[target_word]) * self.cutout_threshold)
...
#                     self.syn0prime[self.prototype_vocab[combined_word].index] +=  alpha * average_vector 
...
#                         print combined_word
            ...
            self.no_of_samples[target_word] += 1
            ...
            if len(values) > 1:
                ...
                m = best_value / second_best_value 
                self.margins[target_word].append(m)
                if m > self.margin_alpha[target_word] and combined_word in self.prototype_vocab:
