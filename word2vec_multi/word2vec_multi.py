...
from numpy import exp, dot, zeros, outer, random, dtype, float32 as REAL,\
    ...
    ndarray, empty, sum as np_sum, prod, average, argmax
...
try:
    ...
    def train_sentence_sg(model, sentence, alpha, work=None):
        ...
        for pos, word in enumerate(sentence):
            ...
            counter = 0
            context_global = zeros(model.layer1_size)
            for pos2, word2 in enumerate(sentence[start : pos + model.window + 1 - reduced_window], start):
                # don't train on OOV words and on the `word` itself
                if word2 and not (pos2 == pos):
                    counter += 1
                    context_global += model.global_syn0[word2.index]
            
            if counter == 0:
                continue
            context_global /= counter
            
            best_sense = argmax( dot (context_global, model.syn0[word.index].T) )
            
            for pos2, word2 in enumerate(sentence[start : pos + model.window + 1 - reduced_window], start):
                # don't train on OOV words and on the `word` itself
                if word2 and not (pos2 == pos):
                    train_sg_pair(model, word, best_sense, word2, alpha, labels)
...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(self, sentences=None, size=100, alpha=0.025, window=5, min_count=5,
        ...
        cbow_mean=0, hashfxn=hash, iter=1, sense_no = 2):
        """
    ...
    def reset_weights(self):
        ...
        self.syn0 = empty((len(self.vocab), self.sense_no, self.layer1_size), dtype=REAL)
        self.global_syn0 =  empty((len(self.vocab), self.layer1_size), dtype=REAL)
        self.context_clusters = zeros((len(self.vocab), self.sense_no, self.layer1_size), dtype=REAL)
        # randomize weights vector by vector, rather than materializing a huge random matrix in RAM at once
        for i in xrange(len(self.vocab)):
            for j in xrange(len(self.sense_no)):
                # construct deterministic seed from word AND seed argument
                # Note: Python's built in hash function can vary across versions of Python
                random.seed(uint32(self.hashfxn(self.index2word[i] + str(j) + str(self.seed))))
                self.syn0[i][j] = (random.rand(self.layer1_size) - 0.5) / self.layer1_size
            random.seed(uint32(self.hashfxn(self.index2word[i] + str(j))))
            self.global_syn0[i] = (random.rand(self.layer1_size) - 0.5) / self.layer1_size
