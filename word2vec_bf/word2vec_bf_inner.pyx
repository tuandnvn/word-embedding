...
cdef unsigned long long fast_sentence1_sg_neg(
    ...
    for d in range(negative+1):
        ...
        if before :
            f = <REAL_t>sdot(&halfsize, &syn0[row1 + halfsize], &ONE, &syn1neg[row2], &ONE)
        else:
            f = <REAL_t>sdot(&halfsize, &syn0[row1], &ONE, &syn1neg[row2 + halfsize], &ONE)
        ...
        if before :
            ...
            saxpy(&halfsize, &g, &syn0[row1 + halfsize], &ONE, &syn1neg[row2], &ONE)
        else:
            saxpy(&halfsize, &g, &syn1neg[row2 + halfsize], &ONE, work, &ONE)
            saxpy(&halfsize, &g, &syn0[row1], &ONE, &syn1neg[row2 + halfsize], &ONE)
    
    if before :
        saxpy(&halfsize, &ONEF, work, &ONE, &syn0[row1 + halfsize], &ONE)
    else:
        saxpy(&halfsize, &ONEF, work, &ONE, &syn0[row1], &ONE)
        ...
        for i in range(sentence_len):
            ...
            for j in range(j, k):
                ...
                if j < i:
                    ...
                    before = 0
                next_random = fast_sentence_sg_neg(negative, table, table_len, syn0, syn1neg, size, indexes[i], indexes[j], _alpha, work, next_random, before)
