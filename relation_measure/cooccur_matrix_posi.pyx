...
def get_cooccur(corpus, word2id, int window, dynamic_window=False):
    ...
    cdef int diff
    cdef list sentence
    logger.info("counting raw co-occurrence counts with relative position")
    cdef np.ndarray[np.float32_t, ndim=2] cooccur = numpy.zeros((len(word2id), len(word2id) * 2 * window), dtype=numpy.float32)
...
#             break
        ...
        for pos in range(s_len):
            ...
            for pos2 in range(max(0, pos - window + reduced_window), min(s_len, pos + window + 1 - reduced_window)):
                ...
                if pos2 > pos:
                    diff = pos2 - pos + window - 1
                else:
                    diff = pos2 - pos + window
                cooccur[id1, id2 * 2 * window + diff] += 1.0
