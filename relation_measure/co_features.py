...
def calculate_cofeatures(dense_vectors, word2id, category_name, list_of_pairs, theshold):
    """
    ...
    for pair in list_of_pairs:
        ...
        if word_1.lower() in word2id and word_2.lower() in word2id:
            ...
            cat_sum += len(set([t for t, val in enumerate(dense_vectors[id_1]) if val > theshold]).\
                           intersection(set([t for t, val in enumerate(dense_vectors[id_2]) if val > theshold])))
            count += 1
...
def calculate_cofeatures_file(dense_vectors, word2id, input_file, theshold=3):
    with codecs.open(input_file, 'r', 'utf-8') as file_handler:
        ...
        for relation in relations:
            calculate_cofeatures(dense_vectors, word2id, relation, relations[relation], theshold)
...
if __name__ == '__main__':
    ...
    dense_vectors = model.word_vectors.todense()
    
    calculate_cofeatures_file(dense_vectors, word2id, outf('original_pairs.txt'))
